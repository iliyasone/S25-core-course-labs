...
This directory contains the infrastructure convergence and deployment logic for Labs 5, 6, 7, and 8.
...
This keeps app-triggered redeploys ordered after the Docker image update instead of racing it. The full convergence playbook also deploys Loki, Promtail, Prometheus, and Grafana on every production push.

Grafana listens on port `3000`, and Prometheus listens on port `9090`. After deployment, open `http://<vm-public-ip>:3000` and sign in with the configured Grafana admin credentials.
